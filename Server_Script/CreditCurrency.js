/**
 * CreditCurrency (Economy 2.4 + Cloud Save 1.4)
 *
 * 목적:
 * - 플레이어 재화를 지급(증가)한다. (server authority)
 * - txnId 기반 멱등 처리(idempotency): 동일 txnId 재호출 시 중복 지급 방지.
...
function _assertString(v, name) {
  if (typeof v !== "string" || v.trim().length === 0) {
    throw new Error(`CreditCurrency: invalid ${name}`);
  }
  return v.trim();
}
...
function _assertPositiveNumber(v, name) {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    throw new Error(`CreditCurrency: ${name} must be a positive number`);
  }
  return n;
}

function _buildIdempotencyKey(playerId, txnId, currencyId) {
  // Cloud Save item key 안전 문자만 사용. ':' 금지.
  return `P_${playerId}_T_${txnId}_C_${currencyId}_CR`;
}
...
module.exports = async ({ params, context, logger }) => {
  ...
  if (!token) {
    throw new Error(`CreditCurrency: missing token for tokenType=${tokenType}. Check context.accessToken/serviceToken.`);
  }
  ...
  try {
    incRes = await currenciesApi.incrementPlayerCurrencyBalance({
      ...
      currencyModifyBalanceRequest: {
        amount, // increment 엔드포인트는 양수 amount를 증가로 해석
        ...(expectedWriteLock !== undefined ? { writeLock: expectedWriteLock } : {}),
      },
    });
  } catch (e) {
    logger.error(`CreditCurrency failed: playerId=${targetPlayerId}, currencyId=${currencyId}, amount=${amount}, txnId=${txnId}`);
    throw e;
  }
  ...
  if (idempotency) {
    // 멱등 레코드 저장 실패는 위험: 중복 지급 가능성이 생김.
    // 따라서 저장 실패는 Warning 로그 + 에러로 중단.
    try {
